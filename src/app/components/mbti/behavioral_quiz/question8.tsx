...
const getReactionText = (num: number): string => {
  const reactionOptions = ['Keep trying(100%)', 'Keep trying(75%)', 'Keep trying(50%)', 'Keep trying(25%)', '0%', '‍Give up(25%)', '‍Give up(50%)', '‍Give up(75%)', '‍Give up(100%)'];
...
const Question8: React.FC<Question8Props> = ({
  handleNext,
  handleBack,
  handleSkip,
  surveyData,
  ...
  const [selectedOption, setSelectedOption] = useState<any>(
    getReactionNumber(surveyData.personality_and_behavior.Decision_Making.toy_out_of_reach) || 5
  );
  ...
  const handleSelectOption = (option: string) => {
    ...
    updateAnswer('personality_and_behavior', 'Decision_Making', 'toy_out_of_reach', getReactionText(parseInt(option)));
  };


  const handleNextPage14 = () => {
    
    if (surveyData.personality_and_behavior.Decision_Making.toy_out_of_reach !== '5' && surveyData.personality_and_behavior.Decision_Making.toy_out_of_reach !== '') {
      handleNext(); 
    } 
  };

  const getNextButtonColor = () => {
    return surveyData.personality_and_behavior.Decision_Making.toy_out_of_reach !== '5' 
    && surveyData.personality_and_behavior.Decision_Making.toy_out_of_reach !== ''
    ...
    <div className="quiz-container">
      ...
      <div className="slider-container desktop" style={{ display: isMobile ? 'none' : 'block' }}>
        ...
        <div className="label-row ">
          <span className="option-label text-[14px] text-[#717680]">🤔Keep trying</span>
          <span className="option-label text-[14px] text-[#717680]">🙂‍Give up</span>
        </div>
      </div>
      ...
      <div className="button-container desktop absolute top-[393px] left-0 right-0 w-[540px] mx-auto flex justify-between">
        ...
        <button className="nav-button next" onClick={handleNextPage14} style={{ background: getNextButtonColor() }}>Next</button>
      </div>
      ...
      <div className="button-container mobile absolute bottom-[48px] left-0 right-0 w-[320px] mx-auto flex justify-between">
        ...
        <button className="nav-button mobile next" onClick={handleNextPage14} style={{ background: getNextButtonColor() }}>
          <svg className="inline md:hidden" xmlns="http://www.w3.org/2000/svg" width="16" height="32" viewBox="0 0 16 32" fill="none">
