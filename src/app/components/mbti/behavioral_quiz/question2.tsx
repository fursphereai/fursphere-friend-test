...
const getReactionText = (num: number): string => {
  const reactionOptions = ['As expected(100%)', 'As expected(75%)', 'As expected(50%)', 'As expected(25%)', '0%', 'Inventive(25%)', 'Inventive(50%)', 'Inventive(75%)', 'Inventive(100%)'];
...
const Question2: React.FC<Question2Props> = ({
  ...
  const [selectedOption, setSelectedOption] = useState<any>(
    getReactionNumber(surveyData.personality_and_behavior.Energy_Socialization.interact_with_toys) || 5
  );
  ...
  const handleSelectOption = (option: string) => {
    ...
    updateAnswer('personality_and_behavior', 'Energy_Socialization', 'interact_with_toys', getReactionText(parseInt(option)));
  };

  const handleNextPage8 = () => {
    
    if (surveyData.personality_and_behavior.Energy_Socialization.interact_with_toys !== '5' && surveyData.personality_and_behavior.Energy_Socialization.interact_with_toys !== '') {
      handleNext(); 
    } 
  };

  const getNextButtonColor = () => {
    return surveyData.personality_and_behavior.Energy_Socialization.interact_with_toys !== '5' 
      && surveyData.personality_and_behavior.Energy_Socialization.interact_with_toys !== ''
    ...
    <div className="quiz-container">
      ...
      <div className="slider-container desktop" style={{ display: isMobile ? 'none' : 'block' }}>
        ...
        <div className="label-row ">
          <span className="option-label text-[14px] text-[#717680]">🎮As expected</span>
          <span className="option-label text-[14px] text-[#717680]">💡Inventive</span>
        </div>
      </div>
      ...
      <div className="button-container desktop absolute top-[393px] left-0 right-0 w-[540px] mx-auto flex justify-between">
        ...
        <button className="nav-button next" style={{ background: getNextButtonColor() }} onClick={handleNextPage8}>Next</button>
      </div>
      ...
      <div className="button-container mobile absolute bottom-[48px] left-0 right-0 w-[320px] mx-auto flex justify-between">
        ...
        <button className="nav-button mobile next" style={{ background: getNextButtonColor() }} onClick={handleNextPage8}>
          <svg className="inline md:hidden" xmlns="http://www.w3.org/2000/svg" width="16" height="32" viewBox="0 0 16 32" fill="none">
