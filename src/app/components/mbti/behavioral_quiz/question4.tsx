...
const getReactionText = (num: number): string => {
  const reactionOptions = ['Strict(100%)', 'Strict(75%)', 'Strict(50%)', 'Strict(25%)', '0%', 'Flexible(25%)', 'Flexible(50%)', 'Flexible(75%)', 'Flexible(100%)'];
...
const Question4: React.FC<Question4Props> = ({
  ...
  const [selectedOption, setSelectedOption] = useState<any>(
    getReactionNumber(surveyData.personality_and_behavior.Routin_Curiosity.prefer_routine) || 5
  );
  ...
  const handleSelectOption = (option: string) => {
    ...
    updateAnswer('personality_and_behavior', 'Routin_Curiosity', 'prefer_routine', getReactionText(parseInt(option)));
  };

  const handleNextPage10 = () => {
    
    if (surveyData.personality_and_behavior.Routin_Curiosity.prefer_routine !== '5' && surveyData.personality_and_behavior.Routin_Curiosity.prefer_routine !== '') {
      handleNext(); 
    } 
  };

  const getNextButtonColor = () => {
    return surveyData.personality_and_behavior.Routin_Curiosity.prefer_routine !== '5' 
    && surveyData.personality_and_behavior.Routin_Curiosity.prefer_routine !== ''
    ...
    <div className="quiz-container">
      ...
      <div className="slider-container desktop" style={{ display: isMobile ? 'none' : 'block' }}>
        ...
        <div className="label-row ">
          <span className="option-label text-[14px] text-[#717680]">🤖Strict</span>
          <span className="option-label text-[14px] text-[#717680]">💓Flexible</span>
        </div>
      </div>
      ...
      <div className="button-container desktop absolute top-[393px] left-0 right-0 w-[540px] mx-auto flex justify-between">
        ...
        <button className="nav-button next" style={{ background: getNextButtonColor() }} onClick={handleNextPage10}>Next</button>
      </div>
      ...
      <div className="button-container mobile absolute bottom-[48px] left-0 right-0 w-[320px] mx-auto flex justify-between">
        ...
        <button className="nav-button mobile next" style={{ background: getNextButtonColor() }} onClick={handleNextPage10}>
          <svg className="inline md:hidden" xmlns="http://www.w3.org/2000/svg" width="16" height="32" viewBox="0 0 16 32" fill="none">
