...
const getReactionText = (num: number): string => {
  const reactionOptions = ['Aggressive(100%)', 'Aggressive(75%)', 'Aggressive(50%)', 'Aggressive(25%)', '0%', 'Friendly(25%)', 'Friendly(50%)', 'Friendly(75%)', 'Friendly(100%)'];
...
const Question3: React.FC<Question3Props> = ({
  ...
  const [selectedOption, setSelectedOption] = useState<any>(
    getReactionNumber(surveyData.personality_and_behavior.Energy_Socialization.stranger_enter_territory) || 5
  );
  ...
  const handleSelectOption = (option: string) => {
    ...
    updateAnswer('personality_and_behavior', 'Energy_Socialization', 'stranger_enter_territory', getReactionText(parseInt(option)));
  };

  const handleNextPage9 = () => {

    if (surveyData.personality_and_behavior.Energy_Socialization.stranger_enter_territory !== '5' && surveyData.personality_and_behavior.Energy_Socialization.stranger_enter_territory !== '') {
      handleNext(); 
    } 
  };

  const getNextButtonColor = () => {
    return surveyData.personality_and_behavior.Energy_Socialization.stranger_enter_territory !== '5' 
    && surveyData.personality_and_behavior.Energy_Socialization.stranger_enter_territory !== ''
  ...
  return (
    ...
    <div className="quiz-container">
      ...
      <div className="button-container desktop absolute top-[393px] left-0 right-0 w-[540px] mx-auto flex justify-between">
        ...
        <button className="nav-button next" style={{ background: getNextButtonColor() }} onClick={handleNextPage9}>Next</button>
      </div>
      ...
      <div className="button-container mobile absolute bottom-[48px] left-0 right-0 w-[320px] mx-auto flex justify-between">
        ...
        <button className="nav-button mobile next" style={{ background: getNextButtonColor() }} onClick={handleNextPage9}>
          <svg className="inline md:hidden" xmlns="http://www.w3.org/2000/svg" width="16" height="32" viewBox="0 0 16 32" fill="none">
