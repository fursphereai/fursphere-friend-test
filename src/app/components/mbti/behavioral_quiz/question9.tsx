...
const getReactionText = (num: number): string => {
  const reactionOptions = ['Exciting(100%)', 'Exciting(75%)', 'Exciting(50%)', 'Exciting(25%)', '0%', '‍Avoidant(25%)', '‍Avoidant(50%)', '‍Avoidant(75%)', '‍Avoidant(100%)'];
...
const Question9: React.FC<Question9Props> = ({
  handleNext,
  handleBack,
  handleSkip,
  surveyData,
  ...
  const [selectedOption, setSelectedOption] = useState<any>(
    getReactionNumber(surveyData.personality_and_behavior.Decision_Making.react_new_friend) || 5
  );
  ...
  const handleSelectOption = (option: string) => {
    ...
    updateAnswer('personality_and_behavior', 'Decision_Making', 'react_new_friend', getReactionText(parseInt(option)));
  };

  const handleNextPage15 = () => {
    
    if (surveyData.personality_and_behavior.Decision_Making.react_new_friend !== '5' && surveyData.personality_and_behavior.Decision_Making.react_new_friend !== '') {
      handleNext(); 
    } 
  };

  const getNextButtonColor = () => {
    return surveyData.personality_and_behavior.Decision_Making.react_new_friend !== '5' 
    && surveyData.personality_and_behavior.Decision_Making.react_new_friend !== ''
    ...
    <div className="quiz-container">
      ...
      <div className="slider-container desktop" style={{ display: isMobile ? 'none' : 'block' }}>
        ...
        <div className="label-row ">
          <span className="option-label text-[14px] text-[#717680]">😆Exciting</span>
          <span className="option-label text-[14px] text-[#717680]">😶‍🌫️Avoidant</span>
        </div>
      </div>
      ...
      <div className="button-container desktop absolute top-[393px] left-0 right-0 w-[540px] mx-auto flex justify-between">
        ...
        <button className="nav-button next" onClick={handleNextPage15} style={{ background: getNextButtonColor() }}>Next</button>
      </div>
      ...
      <div className="button-container mobile absolute bottom-[48px] left-0 right-0 w-[320px] mx-auto flex justify-between">
        ...
        <button className="nav-button mobile next" onClick={handleNextPage15} style={{ background: getNextButtonColor() }}>
          <svg className="inline md:hidden" xmlns="http://www.w3.org/2000/svg" width="16" height="32" viewBox="0 0 16 32" fill="none">
