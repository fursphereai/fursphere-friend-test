...
const getReactionText = (num: number): string => {
  const reactionOptions = ['Confuse(100%)', 'Confuse(75%)', 'Confuse(50%)', 'Confuse(25%)', '0%', 'Keep in mind(25%)', 'Keep in mind(50%)', 'Keep in mind(75%)', 'Keep in mind(100%)'];
...
const Question11: React.FC<Question11Props> = ({
  handleNext,
  handleBack,
  handleSkip,
  surveyData,
  ...
  const [selectedOption, setSelectedOption] = useState<any>(
    getReactionNumber(surveyData.personality_and_behavior.Structure_Spontaneity.respond_to_scold) || 5
  );
  ...
  const handleSelectOption = (option: string) => {
    ...
    updateAnswer('personality_and_behavior', 'Structure_Spontaneity', 'respond_to_scold', getReactionText(parseInt(option)));
  };

  const handleNextPage17 = () => {
    if (surveyData.personality_and_behavior.Structure_Spontaneity.respond_to_scold !== '5' && surveyData.personality_and_behavior.Structure_Spontaneity.respond_to_scold !== '') {
      handleNext(); 
    } 
  };

  const getNextButtonColor = () => {
    return surveyData.personality_and_behavior.Structure_Spontaneity.respond_to_scold !== '5' 
    && surveyData.personality_and_behavior.Structure_Spontaneity.respond_to_scold !== ''
    ...
    <div className="quiz-container">
      ...
      <div className="slider-container desktop" style={{ display: isMobile ? 'none' : 'block' }}>
        ...
        <div className="label-row ">
          <span className="option-label text-[14px] text-[#717680]">😕Confuse</span>
          <span className="option-label text-[14px] text-[#717680]">✍️Keep in mind</span>
        </div>
      </div>
      ...
      <div className="button-container desktop absolute top-[393px] left-0 right-0 w-[540px] mx-auto flex justify-between">
        ...
        <button className="nav-button next" onClick={handleNextPage17} style={{ background: getNextButtonColor() }}>Next</button>
      </div>
      ...
      <div className="button-container mobile absolute bottom-[48px] left-0 right-0 w-[320px] mx-auto flex justify-between">
        ...
        <button className="nav-button mobile next" onClick={handleNextPage17} style={{ background: getNextButtonColor() }}>
          <svg className="inline md:hidden" xmlns="http://www.w3.org/2000/svg" width="16" height="32" viewBox="0 0 16 32" fill="none">
